...
import com.meteor.bukkitioc.annotation.*;
import com.meteor.bukkitioc.exception.NoUniqueBeanDefinitionException;
import com.meteor.bukkitioc.io.PropertyResolver;
import com.meteor.bukkitioc.io.ResourceResolver;
import com.meteor.bukkitioc.util.ClassUtil;
...
public class AnnotationConfigApplicationContext implements ConfigurableApplicationContext{
    ...
    public AnnotationConfigApplicationContext(Class<?> configClass,PropertyResolver propertyResolver,ClassLoader classLoader){
        ...
        // 注入依赖
        this.beans.values().stream().forEach(beanDefinition ->
                injectProperties(beanDefinition,beanDefinition.getAClass(),beanDefinition.getInstance()));
    ...
    public Object createBeanForce(BeanDefinition beanDefinition){
        ...
        System.out.println("处理"+beanDefinition.getName());

        if(beanDefinition.getFactoryMethod()!=null){
            Object bean = getBean(beanDefinition.getFactoryName());
            System.out.println("工厂方法: "+beanDefinition.getFactoryName());
            beanInstance = beanDefinition.getFactoryMethod().invoke(bean,args);
        }else {
            try {
                beanInstance = beanDefinition.getConstructor().newInstance(args);
            }catch (Exception e){
                return null;
            }
        }
...
//                Class<?> aClass = Class.forName(className,true,classLoader.);
                ...
                if(component!=null){
                    ...
                    if(className.equalsIgnoreCase("com/meteor/bukkitiocexample/components/ShopComponent")){
                        if(configuration==null) {
                            System.out.println("configuration anno is null");
                        }
                    }
                        ...
                        for (Method declaredMethod : aClass.getDeclaredMethods()) {
                            ...
                            if(annotation!=null){
                                ...
                                if(Modifier.isPublic(modifiers)){
                                    ...
                                    beanName = annotation.value().isEmpty() ? ClassUtil.getBeanName(declaredMethod) : annotation.value();
                                    BeanDefinition beanDefinitionFactory = new BeanDefinition(beanName
                                            , returnType,
                                            null, null, beanDefinition.getName(),
                                            ...
                                            null, null);
                                    beanDefinitionMap.put(beanName,beanDefinitionFactory);
                                }
                            }
                        }
                    }
    ...
    Set<String> scanForClassName(Class<?> configClass){
        ...
        for (String aPackage : packages) {
            ...
            ResourceResolver resolver = new ResourceResolver(aPackage,classLoader);
